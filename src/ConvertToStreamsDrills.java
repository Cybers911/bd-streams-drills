...
public class ConvertToStreamsDrills {
    ...
    public static List<String> sortDishesByCalories(List<Dish> menu) {
        ...
        return (List<String>) menu.stream()
                .filter(dish -> dish.getCalories() < 400).sorted(Comparator.comparingInt(Dish::getCalories));
    }
    ...
    public static List<Card> newDeck() {
        ...
        return result;*/
        return Stream.of(Suit.values()).flatMap(suit -> Stream.of(Rank.values()).map(rank -> new Card(suit, rank))).collect(Collectors.toList());
    }
}
